...
ALLOWED_ORIGIN = os.getenv('ALLOWED_ORIGIN', 'http://localhost:3002')  # CORS allowed origin
...
class UserDatabase:
    ...
    def verify_user(self, username: str, password: str) -> Optional[Dict]:
        """Verify user credentials against existing SQLite database with robust error handling"""
        max_retries = 3
        retry_delay = 1
        
        for attempt in range(max_retries):
            try:
                # Use connection with timeout and proper error handling
...
def initialize_database():
    """Initialize database with proper setup and error handling"""
    try:
        # Check if database file exists and is accessible
        if os.path.exists(SQLITE_PATH):
            try:
                # Test database connection
                test_conn = sqlite3.connect(SQLITE_PATH, timeout=5)
                test_conn.execute("SELECT 1")
                test_conn.close()
                print(f"Database file exists and is accessible: {SQLITE_PATH}")
            except sqlite3.Error as e:
                print(f"Database file exists but is corrupted: {e}")
                # Create backup of corrupted database
                if os.path.exists(SQLITE_PATH):
                    backup_path = f"{SQLITE_PATH}.corrupted.{int(time.time())}"
                    os.rename(SQLITE_PATH, backup_path)
                    print(f"Corrupted database backed up to: {backup_path}")
                
                # Create fresh database
                create_fresh_database()
                return UserDatabase(SQLITE_PATH)
        else:
            print(f"Database file not found, creating new database: {SQLITE_PATH}")
            create_fresh_database()
            return UserDatabase(SQLITE_PATH)
...
def create_fresh_database():
    ...
    try:
        ...
        default_password_hash = generate_password_hash('admin123', method='pbkdf2:sha256', salt_length=16)
        conn.execute("""
            INSERT OR IGNORE INTO users (username, email, password_hash, display_name, role, is_active)
            VALUES (?, ?, ?, ?, ?, ?)
        """, ('admin', 'admin@stephenasatsa.com', default_password_hash, 'Website Administrator', 'super_admin', 1))
        
        conn.commit()
        conn.close()
        print(f"Fresh database created with default admin user")
...
try:
    ...
    if db:
        ...
        print("Failed to initialize database - running in degraded mode")
        db = None
finally:
    if db is None:
...
def health_check():
    ...
    try:
        ...
        return jsonify({
            ...
            'memory_usage': len(active_sessions),
