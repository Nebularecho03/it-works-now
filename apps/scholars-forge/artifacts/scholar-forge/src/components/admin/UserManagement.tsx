...
export default function UserManagement() {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      if (!token) {
        console.error("[UserManagement] No authentication token found");
        setUsers([]);
        return;
      }
      ...
      if (!response.ok) {
        if (response.status === 403) {
          console.error("[UserManagement] Access denied - admin privileges required");
        } else {
          console.error("[UserManagement] Failed to fetch users:", response.status, response.statusText);
        }
        setUsers([]);
        return;
      }
      ...
      const transformedUsers: User[] = usersData.map((user: any) => ({
        ...
        emailVerified: true, // Default to true since we don't have this field in API
        profileCompleted: !!(user.institution || user.researchInterests || user.bio)
      }));
      
      setUsers(transformedUsers);
      console.log("[UserManagement] Successfully fetched users:", transformedUsers.length);
    } catch (error) {
  ...
  const handleRoleChange = async (userId: string, newRole: "USER" | "ADMIN") => {
    try {
      ...
      if (!response.ok) {
        console.error("[UserManagement] Failed to change role:", response.status, response.statusText);
        return;
      }
      ...
      console.log(`[UserManagement] Successfully changed role for user ${userId} to ${newRole}`);
    } catch (error) {
  ...
  const handleDeleteUser = async (userId: string) => {
    try {
      ...
      if (!response.ok) {
        console.error("[UserManagement] Failed to delete user:", response.status, response.statusText);
        return;
      }

      // Update local state
      setUsers(prev => prev.filter(user => user.id !== userId));
      setSelectedUser(null); // Close the details modal if user was selected
      
      console.log(`[UserManagement] Successfully deleted user ${userId}`);
    } catch (error) {
